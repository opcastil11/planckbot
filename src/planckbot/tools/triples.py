...
class TriplesStore:
    ...
    def get_by_tool(self, tool_name: str, limit: int = 100) -> list[Triple]:
        cur = self.conn.execute(
            "SELECT * FROM triples WHERE tool_name = ? ORDER BY created_at DESC LIMIT ?",
            (tool_name, limit),
        )
    ...
    def count_by_tool(self) -> dict[str, int]:
        cur = self.conn.execute(
            "SELECT tool_name, COUNT(*) as cnt FROM triples GROUP BY tool_name"
        )
    ...
    def list_all(self, limit: int = 100) -> list[Triple]:
        cur = self.conn.execute(
